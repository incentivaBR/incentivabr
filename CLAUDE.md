...
| **Um mecanismo por cliente**, em `organizations.incentive_group_code` (migration 043). A escolha tem fonte única em `lib/mecanismos.js`; nenhuma rota repete o literal do código. Só entra em cliente o mecanismo com teto resolvido (`disponivel_para_cliente`); os demais ficam no catálogo com o motivo escrito (`pendencia_parecer`), porque sem teto próprio `tetoDoMecanismo()` cai no global de 6% e liberaria acima da lei. `/api/config/brand` diz qual lei o site opera | set/2026 | migration 043, `backend/src/lib/mecanismos.js`, `backend/tests/mecanismo-por-tenant.test.mjs` |
