...
| **Um mecanismo por cliente**, em `organizations.incentive_group_code` (migrations 043 e 044). A escolha tem fonte única em `lib/mecanismos.js`; nenhuma rota repete o literal do código. Só entra em cliente o mecanismo com teto resolvido (`disponivel_para_cliente`); os demais ficam no catálogo com o motivo escrito (`motivo_indisponivel`, que pode ser jurídico ou de produto), porque sem teto próprio `tetoDoMecanismo()` cai no global de 6% e liberaria acima da lei. `/api/config/brand` diz qual lei o site opera | set/2026 | migration 043, `backend/src/lib/mecanismos.js`, `backend/tests/mecanismo-por-tenant.test.mjs` |
| Coluna que o código lê tem de existir na tabela: o job do Postgres real cruza toda leitura de `org.x` com as colunas de `organizations`. `org.incentive_group_code` foi lida por quatro rotas sem nunca existir — em JavaScript isso não é erro, e o pg-mem não pega porque o schema dele é escrito à mão no teste | set/2026 | `backend/tests/postgres-real.test.mjs` |
...
| `nucleo.md` é retrato de seis páginas (`FONTES` em `scripts/sync-nucleo-tina.mjs`) e o teste falha se o arquivo não for o que o script geraria: mexeu em página-fonte, roda o script. Do Espaço do Contador entra só o FAQ; a tabela de fichas da DIRPF fica de fora até o tributarista (pergunta 10). "TINA responde" (`tina.yml`) pergunta de verdade em produção, à mão | set/2026 | `scripts/sync-nucleo-tina.mjs`, `backend/tests/nucleo-em-dia.test.mjs`, `.github/workflows/tina.yml` |
...
| Nenhuma página nomeia projeto à mão. Título, descrição, proponente, área e UF vêm do projeto do tenant por `[data-projeto]` (`tenant.js`), com reserva neutra; fora da simulação a descrição cai no resumo do SALIC. Não há foto de projeto até haver campo no cadastro | set/2026 | `frontend/js/tenant.js`, `frontend/projetos-rouanet.html`, `backend/tests/projeto-do-tenant.test.mjs` |
...
| Dinheiro na tela só por `js/moeda.js`: `BRL(v)`, `BRL.inteiro(v)`, valor ausente é "—". Nenhuma página escreve `Intl.NumberFormat` de moeda nem `'R$ ' +`; o teste recusa | set/2026 | `frontend/js/moeda.js`, `backend/tests/moeda.test.mjs` |
| Quem tem conta exerce os direitos da LGPD em `minha-conta.html` (`/api/meus-dados`): exporta em JSON e elimina com a senha. Sem registro fiscal, anonimiza no ato; com registro fiscal (comprovante ou recibo fora da simulação), **encerra** a conta e guarda nome, CPF e destinações até `anoFinalDaGuarda()` (`config/lgpd.js`, ano-base + 1 + 5). `authenticateToken` consulta o banco a cada pedido e recusa conta encerrada. Nada é apagado por prazo: `GET /api/admin/retencao` só lista, até o tributarista confirmar a contagem | set/2026 | migration 042, `backend/src/routes/meusDados.js`, `backend/src/middleware/auth.js`, `backend/tests/meus-dados.test.mjs` |
| Dado de fora (banco, SALIC, formulário) só entra em `innerHTML` embrulhado em `esc()`/`escapeHtml()`; em `href`, `src`, `on*`, `id` e `data-*`, também `encodeURIComponent()`. Guarda estática em `escape-innerhtml.test.mjs` (isenta só página sem fetch) e guarda na tela: o fixture do E2E é envenenado com `"><img data-veneno src=x>` e toda tela é recusada se o veneno virar elemento | set/2026 | `backend/tests/escape-innerhtml.test.mjs`, `backend/tests/servidor-memoria.mjs` (`VENENO`), `backend/scripts/e2e.mjs` |
...
| Cada mecanismo diz o **seu** limite: PRONON e PRONAS/PCD têm 1% **cada**, não 1% dividido. O site afirmava a partilha em quatro páginas, na base da TINA e **no cálculo do validador**, que somava as duas destinações contra um só 1% e acusava excesso onde a lei permite o dobro. Quatro cópias da mesma frase falsa, porque cada página guardava a sua | set/2026 | migration 045, `backend/tests/limites-por-mecanismo.test.mjs`, `docs/juridico/nota-tecnica-pesquisa-2026-09.md` |
| O teto do esporte é **registro, não cálculo**. A LC 222/2025 eleva o conjunto a 7% (a cesta sobe, não se abre uma segunda), e isso está no catálogo (`irpf_global_7`, sem ninguém apontando) e no texto das páginas, marcado como não confirmado. O cálculo segue em `irpf_global_6`: o condicional é lógica de `saldoDisponivel()`, e não se escreve regra fiscal nova sobre documento não assinado. Errar para menos é recuperável no ano seguinte; errar para mais é malha fina | set/2026 | migration 045, `backend/tests/limites-por-mecanismo.test.mjs`, `backend/tests/textos-fiscais.test.mjs` |
| O prazo de guarda termina em **31/12** do ano final, não em 1º de janeiro — ano sozinho é ambíguo e a diferença é um ano de documento apagado cedo. E o prazo **não corre** enquanto houver fiscalização ou processo: o superadmin trava a conta com motivo escrito (migration 046), e travada nunca entra na fila de vencidas. Feito antes de ligar qualquer eliminação, de propósito. Anonimizar a linha não basta: o comprovante e o recibo reidentificam sozinhos, então toda rotina de fim de prazo tem de alcançar os arquivos | set/2026 | migration 046, `backend/src/config/lgpd.js`, `backend/tests/trava-de-retencao.test.mjs` |
| Ler a lei não é ter parecer. Os dispositivos da LC 222/2025 (art. 9º, § 1º, II) e da Lei 12.715/2012 (art. 4º) foram lidos na íntegra em set/2026: o catálogo e as páginas passaram a **citar o dispositivo** no lugar de "fonte secundária", mas `confirmado_por_parecer` seguiu FALSE em tudo. Saber o que o texto diz não é alguém assinar que a nossa aplicação dele está certa | set/2026 | migration 047, `backend/tests/limites-por-mecanismo.test.mjs` |
| **PRONON e PRONAS/PCD em standby.** O caput do art. 4º da Lei 12.715/2012 (redação da Lei 14.564/2023) faculta a dedução à pessoa física só **até o ano-calendário de 2025** — e pessoa física é o público desta plataforma. Sem prorrogação conhecida, não há o que construir: ficam no catálogo e na biblioteca como referência, marcados, e a agenda, o validador e o Espaço do Contador avisam que destinação de 2026 não gera dedução. O teto em si está resolvido (1% cada, fora do teto geral), então prorrogação só exige a jornada | set/2026 | migration 047, `docs/juridico/CONSULTA-TRIBUTARISTA.md` |
| **A data da transferência é dado próprio** (`donations.transferido_em`, migration 048). `created_at` é o registro na plataforma e `confirmed_at` é a conferência do gestor — nenhuma das duas é o dia em que o dinheiro saiu, e é desse dia que correm os prazos do fundo. Contar de qualquer outra erra para mais, que é o lado que faz perder prazo achando que há folga. Informada por quem lê o comprovante: o contribuinte, no upload | set/2026 | migration 048, `backend/src/routes/uploads.js` |
| **Prazo do comprovante é dado do fundo**, não constante: `official_funds.prazo_comprovante_dias`, com órgão e base legal. Quem fixa os 60 dias do FDCA/DF é o CDCA/DF na RN 125/2026, art. 7º; outro conselho fixa outro, e o mesmo conselho muda o seu. Fonte única em `lib/prazos.js`; `null` é informação (a Rouanet não tem janela, porque o proponente emite o recibo), e inventar aviso onde não há prazo treina a pessoa a ignorar o aviso onde há. A conta do fundo **não** entra em migration, mesmo publicada em norma — vem de `org_projects`, por tenant | set/2026 | migration 048, `backend/src/lib/prazos.js`, `backend/tests/prazo-do-comprovante.test.mjs` |
| **Sublimite é fatia, não teto separado** (`incentive_groups.sublimite_pct`, migration 050). O CDCA/DF publica 3% para pessoa física (RN 125/2026, art. 2º, § 1º, II e § 3º) e a plataforma calcula 6%: adotamos os 3% porque é menor sob qualquer leitura e é o número do órgão que emite o recibo. **A armadilha:** dar ao FDCA um `teto_codigo` próprio quebraria o cruzamento de `saldoDisponivel()` e liberaria 6% de Rouanet + 3% de FDCA = 9%. O FDCA continua apontando para `irpf_global_6` e ganha só o sublimite. Vale sempre o **menor** dos dois, e a mensagem de recusa diz qual barrou. Só o FDCA tem: aplicar ao Fundo do Idoso seria inventar limite sem fonte | set/2026 | migration 050, `backend/src/lib/tetos.js`, `backend/tests/sublimite-por-mecanismo.test.mjs` |
| O pg-mem **ignora `FILTER` de agregação sem reclamar** e devolve a soma inteira. Foi assim que o sublimite nasceu errado — a fatia do FDCA aparecia consumida por destinação da Rouanet. Soma condicional no backend usa `SUM(CASE WHEN ...)`, que é portável, e o resultado é conferido contra o Postgres real | set/2026 | `backend/src/lib/tetos.js`, `backend/tests/sublimite-por-mecanismo.test.mjs` |
| **Certificado de Autorização para Captação** (migration 049): no FDCA/DF a OSC só capta com autorização do CDCA/DF, com prazo de dois anos prorrogáveis. São **duas validades** — a da autorização e a do registro da OSC no Conselho —, e a que vence primeiro manda, porque captar exige as duas vivas. O que torna isto mais que um campo de data é o art. 15, § 2º, IV: deixar o prazo extinguir **sem pedir prorrogação** manda os recursos já captados para a universalidade da política. A situação é calculada em `lib/certificado.js`, nunca guardada — guardar "vencido" exigiria alguém virando o campo todo dia. A janela dos seis meses do § 1º é ambígua no texto ("antecedência de até 6 meses"), e a tela avisa a partir dela sem afirmar qual leitura vale: é pergunta para o CDCA/DF, não para o tributarista | set/2026 | migration 049, `backend/src/lib/certificado.js`, `backend/tests/certificado-de-captacao.test.mjs` |
| Material de treinamento de terceiro (o do TCDF, de set/2026) serve para **entender** o processo e nada dele entra no produto, na base da TINA ou no repositório — é obra protegida. O que entra são as normas públicas que ele cita, com o dispositivo. A RN 125/2026 do CDCA/DF é norma pública e está citada no código | set/2026 | migration 048, `backend/src/lib/prazos.js` |
| **O que identifica a destinação é dado do mecanismo** (`incentive_groups.identificador`, migration 051), e o vocabulário também (`laws.termo_identificador/beneficiario/recibo/recibo_emissor`). A rota exigia PRONAC de 6–7 dígitos e procurava o fundo por `WHERE code = 'FNC'`: nenhum cliente fora da Rouanet registrava destinação, e toda destinação era carimbada como FNC. Agora o identificador é explícito (nunca deduzido de "tem pronac preenchido?"), um pronac mandado à toa é **recusado**, o fundo vem de `org_projects` e o título do projeto. A rota atende em `/api/donations/registrar`; `/rouanet` segue vivo. Nenhuma página escreve "PRONAC" ou "Recibo de Mecenato" à mão — vem de `/api/config/brand` por `[data-termo]`, com guarda estática e guarda na tela (E2E compara API × DOM) | set/2026 | migration 051, `backend/src/lib/jornada.js`, `backend/tests/jornada-por-mecanismo.test.mjs`, `backend/scripts/e2e.mjs` |
| Nota de pesquisa não é parecer e não move teto. A de set/2026 respondeu 7 das 11 perguntas e corrigiu três erros de cadastro, mas não tem assinatura: nada virou `confirmado_por_parecer`, nada saiu da simulação, nenhum mecanismo foi liberado. Fica arquivada com a ressalva no topo e com as fontes secundárias que conferimos — o `planalto.gov.br` não é alcançável do ambiente de trabalho | set/2026 | `docs/juridico/nota-tecnica-pesquisa-2026-09.md`, `docs/juridico/CONSULTA-TRIBUTARISTA.md` |
...
| `POST /api/donations/registrar` | registra destinação com validação de teto e sublimite; a jornada (identificador, fundo, vocabulário) vem do mecanismo do cliente. `POST /api/donations/rouanet` é o nome antigo, e responde igual |
...
| `/api/chat` | TINA |
| `GET`/`DELETE /api/meus-dados` | direitos de quem tem conta (LGPD art. 18): exportação em JSON e eliminação com senha; `GET /api/admin/retencao` lista o que venceu o prazo, sem apagar |
