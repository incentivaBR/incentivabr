...
- Com o teto, o sublimite, o prazo e o certificado resolvidos, o FDCA estava pronto no cálculo e **parado na jornada**: duas linhas da rota de registro impediam qualquer cliente que não fosse Rouanet de destinar. A primeira exigia PRONAC de seis ou sete dígitos; a segunda procurava o fundo por `WHERE code = 'FNC'`. Um fundo municipal não tem PRONAC, e carimbar toda destinação como Fundo Nacional de Cultura é gravar o fundo errado no registro que gera o recibo.
- **O que identifica a destinação passou a ser dado do mecanismo** (`incentive_groups.identificador`). Na Rouanet é o PRONAC, um registro externo conferível no SALIC. No FDCA/DF não há número: a RN 125/2026, art. 5º, diz que o doador indica a OSC e, se quiser, o projeto — que é exatamente o que `org_projects` guarda. A coluna é explícita, e não deduzida de "o projeto tem pronac preenchido?": dedução silenciosa é o que faz um cliente mal cadastrado cair na jornada errada sem ninguém perceber.
- **Um identificador mandado à toa é recusado, não ignorado.** Aceitar calado um número que a jornada não usa produz dado que ninguém sabe de onde veio.
- **O fundo vem do projeto do tenant**, com o catálogo do mecanismo como segunda opção e `null` como terceira — nunca um literal. Era o `'FNC'` a razão de o FDCA não poder existir.
- **O vocabulário também é dado.** "PRONAC", "proponente" e "Recibo de Mecenato" são palavras da Rouanet; no FDCA quem recebe é o fundo, quem executa é a OSC e quem emite o recibo é o CDCA/DF — que nem tem conta na plataforma. Os quatro termos ficam em `laws`, ao lado de `orgao` e `sistema_oficial`, e chegam à tela por `/api/config/brand`. Nenhuma página escreve essas palavras à mão, pela mesma razão que nenhuma escreve percentual: quatro cópias de uma frase viram quatro frases diferentes.
- **O Fundo da Pessoa Idosa ficou com o termo genérico** em vez de herdar o do CDCA/DF. Tem conselho e norma próprios, que não lemos; nomear o emissor errado num documento fiscal é pior do que não nomear.
- A rota ganhou o endereço `/api/donations/registrar`, que é o que ela sempre foi. `/rouanet` continua respondendo igual — o nome antigo está em link, em e-mail e no navegador de quem já usou, e quebrá-lo não melhora nada.
- **Duas guardas na tela, porque é lá que a palavra aparece.** Um teste estático recusa "PRONAC" ou "Recibo de Mecenato" escritos no JavaScript do assistente fora do texto de reserva único; e o fluxo no Chromium compara o que `/api/config/brand` respondeu com o que está na tela — se a ponte quebrar, aqui ainda parece certo (a reserva é a da Rouanet) e erra no primeiro cliente que não for Rouanet.
- **O job do Postgres real ganhou a guarda do vocabulário.** O código lê `laws.termo_*` e `incentive_groups.identificador` por JOIN: coluna que não existe derruba a consulta, e coluna vazia cai no texto de reserva sem reclamar. As duas falhas passam no pg-mem, cujo schema é escrito à mão no teste.
...
- **O art. 15, § 2º, IV é o motivo de tudo isto existir:** desistir formalmente OU **deixar extinguir o prazo sem pedido de prorrogação** manda os recursos já captados para a universalidade da política distrital. A OSC perde para o fundo geral o dinheiro que levantou — por decurso de prazo, sem que ninguém precise fazer nada.
- **São duas validades, e a que vence primeiro manda.** A autorização (art. 14, V) e o registro da OSC no CDCA/DF (art. 14, IV) vencem em datas diferentes, e captar exige as duas vivas. A tela diz **qual** venceu: chamar de "autorização vencida" o que foi o registro da entidade seria mentira, e um teste recusa atribuir ao registro a consequência que é da autorização.
- **A situação é calculada, nunca guardada.** Guardar "vencido" no banco exigiria alguém rodando todo dia para virar o campo. `lib/certificado.js` é a fonte única, e a tela do gestor mostra verde, amarelo ou vermelho com a frase pronta.
- **A rota de prorrogação existe porque o art. 15 é sobre isso.** Sem `PUT /api/admin/orgs/:id/projects/:projectId`, prorrogar exigiria cadastrar o projeto de novo — duplicando o histórico de captação do projeto que continua sendo o mesmo. A rota mexe só no bloco do certificado: um endpoint que atualiza tudo de uma vez é um endpoint que apaga dado bancário por engano. E confere o cliente: o id do projeto sozinho permitiria mexer no certificado de outro tenant.
- **Uma ambiguidade que não fingimos resolver.** O § 1º fala em requerer a prorrogação "com antecedência de até 6 (seis) meses do fim do prazo", o que comporta duas leituras — a janela abre seis meses antes, ou o pedido precisa de no mínimo seis meses. A diferença decide se uma OSC com três meses restantes ainda pode pedir. Não é pergunta de tributarista, é do próprio CDCA/DF; a tela avisa a partir dos seis meses e manda procurar o Conselho, sem afirmar qual leitura vale.
- `meta_captacao` entra junto, e vem da planilha orçamentária do art. 12, III — não do certificado, que não lista valor. Entra porque o art. 15, § 2º trata inteiro do que fazer quando a captação não alcança o necessário.
- **A guarda de escape pegou uma interpolação minha:** eu montava a cor do aviso dentro do `innerHTML`. Virou classe de CSS — sem interpolação, a regra fica intacta e o código melhora.
